...
| Base compose stack pins tier34 to port 3001 with CORS `http://localhost:5173` | UI and API origin pairing must match client defaults | `docker-compose.yml` `tier34.environment`: `TIER34_PORT: "3001"`, `TIER34_CORS_ORIGIN: "http://localhost:5173"` | **Medium** — remote UI or overlay HTTPS gateway requires env overrides |
