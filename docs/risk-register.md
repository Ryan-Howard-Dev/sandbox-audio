...
| R-003 | **JS queue vs Exo `MediaItem` desync** — dual queue authority; index reconciliation via URL matching | Playback & Queue, Audio | High | High | `playback-queue-analysis.md` Interpretation §2; `playback-queue-invariants.md` `findQueueIndexForExoUrl`; `architecture-violations.md` dual queue authority; `audio-analysis.md` Verified Fact §6 | **Done:** native queue items carry a stable `MediaItem.mediaId`, emitted on every transition event; `exoQueueSync.findQueueIndexForExoTransition` reconciles on that key and only falls back to URL matching. Remaining: single advance owner per platform path |
| R-004 | **JS `subscribeEnded` vs native gapless auto-advance race** — double-advance or stale index when suppress window fails | Playback & Queue, Audio | Medium | High | `architecture-violations.md`; `playback-queue-invariants.md` suppress window; `audio-analysis.md` JS FSM vs native desync risk | **Done:** native→JS transition payload now carries `mediaId` beyond URL. Remaining: extend `shouldSuppressJsAdvanceAfterNativeGapless` coverage — the double-advance race itself is untouched |
